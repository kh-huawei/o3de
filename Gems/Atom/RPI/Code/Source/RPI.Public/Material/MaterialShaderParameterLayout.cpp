...
namespace AZ
{
    namespace RPI
    {
        ...
        bool MaterialShaderParameterLayout::IsPropertyTypeCompatibleWithSrg(
            const MaterialShaderParameterDescriptor* desc,
            const MaterialPropertyDataType dataType,
            const RHI::ShaderResourceGroupLayout* srgLayout)
        {
            bool isValid = false;

            // we can only differentiate between image and constant for the SRG
