    ...
    namespace Render
    {
        ...
        public:
            ...
            static void ForEachLodMesh(
                ...
                AZStd::function<bool(
                    const uint32_t,
                    const uint32_t,
                    DeferredMaterialFeatureProcessor::ModelData&,
                    DeferredMaterialFeatureProcessor::MeshData&)> callback)
            {
            ...
            const ModelId& modelId, ModelDataInstanceInterface* meshHandle, const Data::Instance<RPI::Model>& model)
        {
            ...
            MeshIterator::ForEachLodMesh(
                ...
                [&](const uint32_t lod,
                    const uint32_t meshIndex,
                    const RPI::ModelLodAsset::Mesh& assetMesh,
        ...
        void DeferredMaterialFeatureProcessor::UpdateMaterials(
            const ModelId& modelId, ModelDataInstanceInterface* meshHandle, const Data::Instance<RPI::Model>& model)
        {
            AZStd::scoped_lock lock(m_updateMutex);
            ...
            if (modelData.m_lodData.empty())
            {
                AZ_Assert(
                    false,
                    "DeferredMaterialFeatureProcessor::UpdateMaterials() called for unknown model %s",
                    modelId.ToFixedString().c_str());
            }
            ...
            MeshIterator::ForEachLodMesh(
                    ...
                    const RPI::ModelLod::Mesh& mesh)
                {
                    ...
                    auto material = customMaterialInfo.m_material ? customMaterialInfo.m_material : mesh.m_material;

                    modelData.m_lodData[lod].m_meshData[meshIndex].m_meshDrawPacket.UpdateMaterial(material);
                    m_needsUpdate = true;
                    return true;
                });
        }
        ...
        void DeferredMaterialFeatureProcessor::UpdateMeshDrawPackets()
        {
            for (auto& [modelId, modelData] : m_modelData)
            {
                MeshIterator::ForEachLodMesh(
                    ...
                    [&](const uint32_t lod,
                        ...
                        DeferredMaterialFeatureProcessor::MeshData& meshData)
                    {
                        meshData.m_meshDrawPacket.Update(GetParentScene(), &m_drawPacketManager);
                        return true;
                    });
            }
        }
        ...
        void DeferredMaterialFeatureProcessor::Render(const RenderPacket& renderPacket)
        {
            ...
            for (const AZ::RPI::ViewPtr& view : renderPacket.m_views)
            {
                if (view->GetUsageFlags() & AZ::RPI::View::UsageCamera)
                {
                    ...
                    {
                        view->AddDrawPacket(drawPacket->GetRHIDrawPacket());
                    }
                }
            }
        }
